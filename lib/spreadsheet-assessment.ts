...
import { sanitizeString, sanitizeQuestionNumber } from "./sanitize"
...
export function isQuestionRow(row: any[]): boolean {
  ...
  const isGuidance =
    questionText.includes("In this section") ||
...
function isTitleRow(row: any[]): boolean {
  ...
  const titlePatterns = [/cyber essentials/i, /question set/i, /assessment/i, /questionnaire/i]
...
export function detectAnswerColumn(data: any[][], startRow = 0, endRow = 50): number {
...
export function getColumnHeaders(data: any[][], headerRowIndex = 0): string[] {
...
export function detectColumns(data: any[][]): ColumnDetectionResult {
  ...
  for (let i = dataStartRow; i < Math.min(dataStartRow + 50, data.length); i++) {
    ...
    for (let j = 0; j < row.length; j++) {
      ...
      if (cell.includes("question") && !cell.includes("no") && !cell.includes("type") && questionTextCol === -1) {
        questionTextCol = j
      }

      if (cell.includes("answer type") || cell.includes("response type") || (cell === "type" && answerTypeCol === -1)) {
        answerTypeCol = j
      }
    }
...
export function parseSpreadsheet(data: any[][], userAnswerColOverride?: number): SpreadsheetQuestion[] {
  ...
  const tryParse = (
    ...
    for (let i = startRow + 1; i < data.length; i++) {
      ...
      const mappedRow = [row[qNoCol], row[qTextCol], row[aTypeCol], row[uAnswerCol]]

      if (isQuestionRow(mappedRow)) {
        const rawQuestionNo = String(row[qNoCol] || "").trim()
        const questionNo = sanitizeQuestionNumber(extractQuestionNumber(rawQuestionNo))
        const questionText = sanitizeString(String(row[qTextCol] || "").trim())
        const answerType = sanitizeString(String(row[aTypeCol] || "Notes").trim())
        const userAnswer = sanitizeString(String(row[uAnswerCol] || "").trim())
...
export function parseTextQuestions(text: string): SpreadsheetQuestion[] {
  ...
  while (i < lines.length) {
    ...
    if (line.includes("\t")) {
      ...
      if (parts.length >= 2) {
        questionNo = sanitizeQuestionNumber(extractQuestionNumber(parts[0]))

        const remainingText = parts.slice(1).join(" ").trim()
        const answerSplit = remainingText.match(/^(.+?)[.\s]*(?:Answer:|Response:)\s*(.+)$/i)

        if (answerSplit) {
          questionText = sanitizeString(answerSplit[1].trim())
          userAnswer = sanitizeString(answerSplit[2].trim())
        } else {
          questionText = sanitizeString(parts[1] || "")
          answerType = sanitizeString(parts[2] || "Notes")
          userAnswer = sanitizeString(parts[3] || "")
        }
      }
    }
    ...
    // Try simple format: "A1.1: Question? Answer: text" (single line)
      ...
      if (questionMatch) {
        ...
        questionNo = sanitizeQuestionNumber(extractedNo.match(/^[A-Z]/) ? extractedNo : extractedNo)
        ...
        if (answerSplit) {
          ...
          questionText = sanitizeString(remaining)

          if (i + 1 < lines.length) {
            const nextLine = lines[i + 1]
            const answerMatch = nextLine.match(/^(?:Answer:|Response:)\s*(.+)$/i)
            if (answerMatch) {
              userAnswer = sanitizeString(answerMatch[1].trim())
              i++
            }
          }
        }
    ...
    // Try format without colon: "A1.8 Question text here"
      ...
      if (questionMatch) {
        const extractedNo = questionMatch[1].trim()
        questionNo = sanitizeQuestionNumber(extractedNo.match(/^[A-Z]/) ? extractedNo : extractedNo)
        questionText = sanitizeString(questionMatch[2].trim())
...
export function assessQuestion(question: SpreadsheetQuestion): FlaggedIssue | null {
  ...
  if (answerType.toLowerCase().includes("yes")) {
    ...
    if (normalizedAnswer.includes("no")) {
      if (
        questionText.includes("firewall") ||
...
export function generateAssessment(questions: SpreadsheetQuestion[]): AssessmentResult {
  ...
  for (const question of questions) {
    ...
    section.score = section.totalQuestions > 0 ? (section.answeredQuestions / section.totalQuestions) * 100 : 0
  }
